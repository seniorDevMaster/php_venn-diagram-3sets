        ...
        function chart(selection) {
            ...
            var enter = nodes.enter()
                ...
                .attr("class", function(d) {
                    var className = "venn-area venn-" + (d.sets.length == 1 ? "circle" : "intersection")
                    return className;
                })
            ...
            for (var i = 0; i < data.length; ++i) {
                if (data[i].dots > 0) {                 
                    ...
                    for (let j = 0; j < data[i].dots; j++) {
                        ...
                        .attr("r", "5")
                        .attr('class', 'node')
                        .attr('keyword', data[i].tips[j])
                        .attr("data-venn-sets", function(d) {
                            return d[i].sets.join(",");
                        })
                    }
                }
            }
            ...
            for (var i=0; i<allCircle.length; ++i) {
                var setName= allCircle[i].attributes['data-venn-sets'].value
                if (setNameTemp.length !=0)
                    if (setNameTemp[0] == setName)
                        repeat = true;
                    else {
                        repeat = false;
                        setNameTemp.pop()
                    ...
                    setNameTemp.push(setName)

                if (!repeat) {
                    allCircle[i].setAttribute('cy', textCentres[setName].y)
                    allCircle[i].setAttribute('cx', textCentres[setName].x)

                } else {
                    var alpa = Math.random() * 360 + 1
                    if (setName.length > 15) {
                        var distance = Math.floor(Math.random() * 101)
                    } else if (setName.length <= 12) {
                        var distance = Math.floor(Math.random() * 151)
                    }
                    
                    allCircle[i].setAttribute('cy', textCentres[setName].y + Math.sin(alpa) * distance)
                    allCircle[i].setAttribute('cx', textCentres[setName].x + Math.cos(alpa) * distance)
                }

                allCircle[i].addEventListener("mousemove", function(event) {
                    tooltip.style("left", (event.pageX) + "px")
                        .style("top", (event.pageY - 28) + "px");
                })

                allCircle[i].addEventListener("mouseover", function(d, i) {
                    d3.select(this).moveParentToFront()
                        .transition()
                        .style("stroke-opacity", 1);
    
                    tooltip.transition().style("opacity", .9);
                    tooltip.text(this.getAttribute('keyword'));
                })
                allCircle[i].addEventListener("mouseout", function(d, i) {
            ...
            var updateText = update.selectAll("text")
                ...
                .attr("x", function(d, loop) { 
                    if (loop == 0) {
                        loop++;
                        return Math.floor(textCentres[d.sets].x+50);
                    }
                    if (loop == 1) {
                        loop++;
                        return Math.floor(textCentres[d.sets].x-50);
                ...
                .attr("y", function(d, loop) { 
                    if (loop == 0) {
                        loop++;
                        return Math.floor(textCentres[d.sets].y+150);
                    }
                    if (loop == 1) {
                        loop++;
                        return Math.floor(textCentres[d.sets].y+150);
                    ...
                    if (loop == 2) {
                        loop++;
                        return Math.floor(textCentres[d.sets].y-120);
                    }
